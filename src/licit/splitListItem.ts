...
function splitEmptyListItem(tr: Transform, schema: Schema): Transform {
  ...
  const listFound =
    (orderedListType &&
      findParentNodeOfType(orderedListType)((tr as Transaction).selection)) ||
    (bulletListType &&
      findParentNodeOfType(bulletListType)((tr as Transaction).selection));
  ...
  tr = (tr as Transaction).setSelection(
    TextSelection.create(tr.doc, insertFrom)
  );
  return tr;
}
