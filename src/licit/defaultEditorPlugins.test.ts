...
jest.mock('./plugins/contentPlaceholderPlugin', () =>
  jest.fn(() => new Plugin({}))
);
jest.mock('./plugins/cursorPlaceholderPlugin', () =>
  jest.fn(() => new Plugin({}))
);
jest.mock('./plugins/editorPageLayoutPlugin', () =>
  jest.fn(() => new Plugin({}))
);
...
describe('DefaultEditorPlugins', () => {
  ...
  test('should initialize with the expected plugins', () => {
    ...
    expect(setPluginKey).toHaveBeenCalledWith(
      expect.any(Plugin),
      'EditorKeyMap'
    );
  });
