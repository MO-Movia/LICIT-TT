...
import TableAddRowBeforeCommand from './tableAddRowBeforeCommand';
import { Schema } from '@tiptap/pm/model';

// Mock Editor (used by TableAddRowBeforeCommand)
jest.mock('@tiptap/react', () => {
  return {
    Editor: jest.fn().mockImplementation(() => ({
      commands: {
        addRowBefore: jest.fn(),
      },
    })),
  };
});

describe('TableAddRowBeforeCommand', () => {
  let command: TableAddRowBeforeCommand;
  ...
  beforeEach(() => {
    ...
    // Mock the editor instance to check if the addRowBefore method gets called
    mockEditor = new Editor();
    command = new TableAddRowBeforeCommand();
    mockState = {
      doc: dummyDoc,
      schema: mySchema,
      selection: {},
  ...
  it('should call addRowBefore when execute is called', () => {
    // Mock the addRowBefore command method
    const addRowBeforeMock = jest.spyOn(command, 'getEditor').mockReturnValue({
      commands: {
        addRowBefore: () => {
          return true;
        },
    ...
    // Verify that addRowBefore was called
    expect(addRowBeforeMock).toHaveBeenCalled();
  });

  it('should return the result of addRowBefore when execute is called', () => {
    // Mock the return value of addRowBefore
