...
// Mock Editor (used by tableMoveToPreviousCellCommand)
jest.mock('@tiptap/react', () => {
  return {
    Editor: jest.fn().mockImplementation(() => ({
      commands: {
        goToPreviousCell: jest.fn(),
      },
    })),
  };
});
...
describe('tableMoveToPreviousCellCommand', () => {
  ...
  it('should call goToPreviousCell when execute is called', () => {
    // Mock the goToPreviousCell command method
    const goToPreviousCellMock = jest
    ...
    expect(goToPreviousCellMock).toHaveBeenCalled();
  });

  it('should return the result of goToPreviousCell when execute is called', () => {
    // Mock the return value of goToPreviousCell
