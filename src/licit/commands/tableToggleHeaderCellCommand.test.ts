...
import tableToggleHeaderCellCommand from './tableToggleHeaderCellCommand';
import { Schema } from '@tiptap/pm/model';

// Mock Editor (used by tableToggleHeaderCellCommand)
jest.mock('@tiptap/react', () => {
  return {
    Editor: jest.fn().mockImplementation(() => ({
      commands: {
        toggleHeaderCell: jest.fn(),
      },
    })),
  };
});

describe('tableToggleHeaderCellCommand', () => {
  let command: tableToggleHeaderCellCommand;
  ...
  beforeEach(() => {
    ...
    // Mock the editor instance to check if the toggleHeaderCell method gets called
    mockEditor = new Editor();
    command = new tableToggleHeaderCellCommand();
    mockState = {
      doc: dummyDoc,
      schema: mySchema,
      selection: {},
  ...
  it('should call toggleHeaderCell when execute is called', () => {
    // Mock the toggleHeaderCell command method
    const toggleHeaderCellMock = jest
    ...
    expect(toggleHeaderCellMock).toHaveBeenCalled();
  });

  it('should return the result of toggleHeaderCell when execute is called', () => {
    // Mock the return value of toggleHeaderCell
