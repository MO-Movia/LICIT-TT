...
import tableDeleteColumnCommand from './tableDeleteColumnCommand';
import { Schema } from '@tiptap/pm/model';

// Mock Editor (used by tableDeleteColumnCommand)
...
describe('tableDeleteColumnCommand', () => {
  let command: tableDeleteColumnCommand;
  ...
  beforeEach(() => {
    ...
    // Mock the editor instance to check if the deleteColumn method gets called
    mockEditor = new Editor();
    command = new tableDeleteColumnCommand();
    mockState = {
      doc: dummyDoc,
      schema: mySchema,
      selection: {},
  ...
  it('should call deleteColumn when execute is called', () => {
    // Mock the deleteColumn command method
    const deleteColumnMock = jest.spyOn(command, 'getEditor').mockReturnValue({
      commands: {
        deleteColumn: () => {
          return true;
        },
    ...
    // Verify that deleteColumn was called
    expect(deleteColumnMock).toHaveBeenCalled();
  });

  it('should return the result of deleteColumn when execute is called', () => {
    // Mock the return value of deleteColumn
