...
// Mock Editor (used by tableMoveToNextCellCommand)
...
describe('tableMoveToNextCellCommand', () => {
  ...
  it('should call goToNextCell when execute is called', () => {
    // Mock the goToNextCell command method
    const goToNextCellMock = jest.spyOn(command, 'getEditor').mockReturnValue({
      commands: {
        goToNextCell: () => {
          return true;
        },
    ...
    expect(goToNextCellMock).toHaveBeenCalled();
  });

  it('should return the result of goToNextCell when execute is called', () => {
    // Mock the return value of goToNextCell
