...
type CreateTableConfig = {
  content: HTMLElement | { querySelector?: (selector: string) => Element | null };
  tocElement: string;
  tofElement?: string;
  totElement?: string;
  titleElements: string[];
  titleElementsTOF?: string[];
  titleElementsTOT?: string[];
};

export function createTable(config: CreateTableConfig): void {
  const {
    content,
    tocElement,
    tofElement,
