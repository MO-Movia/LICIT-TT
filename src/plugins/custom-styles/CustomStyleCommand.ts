...
import { Mark, Node, Fragment, Schema, MarkType } from 'prosemirror-model';
...
// Per-styleName caches for commands and marks. Cleared via
// registerStyleCacheInvalidator when the style list changes.
...
function getCachedStyleCommands(
  styleName: string,
  customStyle
): UICommand[] {
  if (!styleName) {
...
// Shallow attribute equality check — used to detect no-op style rewrites.
function shallowAttrsEqual(
...
// Check if a paragraph node's text children already have exactly the
// expected marks (plus any preserved link/override/spacer marks). If so,
// the remove-all-marks + re-add cycle can be skipped entirely.
...
export class CustomStyleCommand extends UICommand {
  ...
  hasCustomStyle(node: Node): boolean {
    return (
      !!node.attrs.styleName && node.attrs.styleName !== RESERVED_STYLE_NONE
    );
  }
      ...
      saveStyle(val)
        .then((result) => {
          ...
          if (!styleHasNumbering(val) || isValidHeirarchy(val.styleName, 0)) {
            // to add previous hierarchy levels
...
function createStyleMark(property: string, value, schema: Schema): Mark | null {
  switch (property) {
    case STRONG:
    case BOLDPARTIAL:
    ...
    case FONTSIZE:
      return schema.marks[MARKFONTSIZE]?.create(value ? { pt: value } : null);
    case FONTNAME:
      return schema.marks[MARKFONTTYPE]?.create(value ? { name: value } : null);
    case TEXTHL:
...
function executeStyleCommand(
  ...
  const returnVal: unknown = handler.call(element, state, tr, startPos, endPos);
...
function hasMismatchHeirarchy(
  ...
  let hasHeirarchyBroken =
    evaluatePreviousHierarchy(
      attrs,
      startPos,
      styleLevel,
      previousLevel,
...
function getLastHierarchyLevel(nodeArray: NodeWithPos[]): number | null {
  ...
  return Number(getStyleLevel(nodeArray.at(-1).node.attrs.styleName));
}
...
function applyBoldPartialStyleToNode(
  ...
  return addMarksToLine(tr, state, node, pos, styleProp.styles.boldSentence);
}
