...
jest.mock('./customStyle', () => ({
  ...
  addStyleToList: jest.fn((style) => [style]),
  setStyles: jest.fn(),
}));

jest.mock('./clearCustomStyleMarks', () => ({
  removeTextAlignAndLineSpacing: jest.fn((tr) => tr),
...
jest.mock('./ParagraphSpacingCommand', () => {
  class ParagraphSpacingCommand {
    ...
    executeCustomStyleForTable(_s, tr) {
      return tr;
    }
  }
...
jest.mock('../../commands', () => {
  ...
  return {
    ...
    getLineSpacingValue: jest.fn((value) => value),
...
describe('CustomStyleCommand', () => {
  ...
  it('calls saveStyle when createNewStyle is invoked with valid style', () => {
    const command = new csc.CustomStyleCommand({ styleName: 'NewStyle' }, 'NewStyle');
  ...
  it('retrieves custom styles and sets them', () => {
    ...
    const command = new csc.CustomStyleCommand({ styleName: 'TestStyle' }, 'TestStyle');
...
describe('CustomStyleCommand - execute mode branches', () => {
  it('handles newstyle mode by opening edit window', () => {
    const command = new csc.CustomStyleCommand('newstyle' as any, 'NewStyle');
    const state = makeState();
    const view = { focus: jest.fn() } as any;
    jest.spyOn(command, 'editWindow' as any);
  ...
  it('handles editall mode without ctrl key', () => {
    const command = new csc.CustomStyleCommand('editall' as any, 'EditAll');
    const state = makeState();
    const view = { focus: jest.fn() } as any;
  ...
  it('handles clearstyle mode', () => {
    const command = new csc.CustomStyleCommand('clearstyle' as any, 'ClearStyle');
    const state = makeState();
    const dispatch = jest.fn();
    jest.spyOn(command, 'executeClearStyle' as any);
  ...
  it('handles reset mode', () => {
    const command = new csc.CustomStyleCommand('reset' as any, 'Reset');
    const state = makeState();
    jest.spyOn(command, 'resetNumber' as any);
  ...
  it('isStyleEnabled returns true for non-clearstyle menus', () => {
    const command = new csc.CustomStyleCommand({ styleName: 'Test' }, 'Test');
    const state = makeState();
    jest.spyOn(command, 'isCustomStyleApplied' as any).mockReturnValue('AppliedStyle');

    const result = command.isStyleEnabled(state, {} as any, 'somemenu');
  ...
  it('isStyleEnabled returns false for clearstyle with no applied style', () => {
    const command = new csc.CustomStyleCommand({ styleName: 'Test' }, 'Test');
    const state = makeState();
    jest.spyOn(command, 'isCustomStyleApplied' as any).mockReturnValue('RESERVED_STYLE_NONE');

    const result = command.isStyleEnabled(state, {} as any, 'clearstyle');
  ...
  it('isStyleEnabled returns true for clearstyle with applied style', () => {
    const command = new csc.CustomStyleCommand({ styleName: 'Test' }, 'Test');
    const state = makeState();
    jest.spyOn(command, 'isCustomStyleApplied' as any).mockReturnValue('CustomStyle');
...
describe('CustomStyleCommand - basic instance methods', () => {
  ...
  it('waitForUserInput resolves with undefined', async () => {
    const command = new csc.CustomStyleCommand({ styleName: 'X' }, 'X');
    await expect(command.waitForUserInput()).resolves.toBeUndefined();
  });

  it('executeWithUserInput returns false', () => {
    const command = new csc.CustomStyleCommand({ styleName: 'X' }, 'X');
    expect(command.executeWithUserInput()).toBe(false);
  });
  ...
  it('executeCustom returns the supplied transform unchanged', () => {
    const command = new csc.CustomStyleCommand({ styleName: 'X' }, 'X');
    const state = makeState();
    const tr = state.tr;
  ...
  it('executeCustomStyleForTable returns the supplied transform unchanged', () => {
    const command = new csc.CustomStyleCommand({ styleName: 'X' }, 'X');
    const state = makeState();
    const tr = state.tr;
    expect(command.executeCustomStyleForTable(state, tr)).toBe(tr);
  });

  it('renderLabel returns the custom style name', () => {
    const command = new csc.CustomStyleCommand({ styleName: 'X' }, 'MyLabel');
    expect(command.renderLabel()).toBe('MyLabel');
  });
  ...
  it('isEmpty returns false for a non-empty object', () => {
    const command = new csc.CustomStyleCommand({ styleName: 'X' }, 'X');
    expect(command.isEmpty({ key: 'value' })).toBe(false);
  });

  it('createCustomObject returns an object with style data', () => {
    ...
    const view = {} as any;
    const result = command.createCustomObject(view, 1);
    expect(result.styleName).toBe('NewName');
...
describe('getStyleLevel branches', () => {
  it('returns 0 when styleName is undefined', () => {
    expect(csc.getStyleLevel(undefined as any)).toBe(0);
  });
...
describe('isStyleEnabled branches', () => {
  it('returns false when clearstyle and applied style equals RESERVED_STYLE_NONE', () => {
    ...
    const state = makeState();
    jest
      .spyOn(command, 'isCustomStyleApplied' as any)
      .mockReturnValue(RESERVED_STYLE_NONE);
    expect(command.isStyleEnabled(state, {} as any, 'clearstyle')).toBe(true);
  });
  ...
  it('returns true for non-clearstyle menu regardless of applied style', () => {
    ...
    jest
      .spyOn(command, 'isCustomStyleApplied' as any)
      .mockReturnValue(RESERVED_STYLE_NONE);
    expect(command.isStyleEnabled(state, {} as any, 'somemenu')).toBe(true);
  });
});
...
describe('resetNumber', () => {
  it('sets the reset attribute and dispatches', () => {
    ...
    const state = makeState();
    const dispatch = jest.fn();
    const newattrs: any = { styleName: 'X' };
  ...
  it('handles missing newattrs without throwing', () => {
    ...
    const dispatch = jest.fn();
    expect(() =>
      command.resetNumber(state, dispatch, 0, undefined)
    ).not.toThrow();
  });
});
...
describe('execute - reset mode and editall with ctrl key', () => {
  it('execute returns false in reset mode', () => {
    const command = new csc.CustomStyleCommand('reset' as any, 'Reset');
    const state = makeState();
    const result = command.execute(state, jest.fn(), undefined);
  ...
  it('editall with ctrl key opens the JSON editor', () => {
    ...
    const jsonEditorSpy = jest.spyOn(command, 'jsonEditor' as any);
    command.execute(state, jest.fn(), view, event);
...
describe('jsonEditor and editWindow', () => {
  ...
  it('editWindow opens the custom style editor popup', () => {
    const command = new csc.CustomStyleCommand({ styleName: 'X' }, 'X');
    const state = makeState();
    const view = { focus: jest.fn(), dispatch: jest.fn() } as any;
    expect(command._popUp).toBeNull();
...
describe('removeAllMarksExceptLink', () => {
  ...
  it('handles undefined transform safely', () => {
    expect(() =>
      csc.removeAllMarksExceptLink(0, 10, undefined as any)
    ).not.toThrow();
  });
});
...
describe('removeAllMarksExceptLinkForTableColumnCell', () => {
  ...
  it('returns the transform when node is not a paragraph', () => {
    const state = makeState();
    const tr = state.tr;
    const fakeNode = { type: { name: 'heading' }, childCount: 0 } as any;
    const result = csc.removeAllMarksExceptLinkForTableColumnCell(
      0,
      fakeNode,
      tr
...
describe('isLevelUpdated', () => {
  it('returns false when the custom style is not already applied in the doc', () => {
    ...
    const state = makeState();
    const result = csc.isLevelUpdated(state, 'X', { styles: {} } as any);
    expect(result).toBe(false);
  });
...
describe('updateDocument', () => {
  ...
  it('applies latest style when a matching paragraph exists', () => {
    ...
    const tr = csc.updateDocument(
      state,
      state.tr,
      'UpdateStyle',
      { styles: { strong: true } } as any
    );
    expect(tr).toBeDefined();
  });
});
...
describe('executeClearStyle - dispatch behaviour', () => {
...
describe('applyLatestStyle', () => {
  it('applies the style to a paragraph node', () => {
    ...
    const result = csc.applyLatestStyle(
      'X',
      state,
      state.tr,
      context,
...
describe('applyStyle - selection variants', () => {
  it('applies style across a regular text selection', () => {
    ...
    const state = makeState();
    const style = { styles: { strong: true } } as any;
...
describe('addMarksToLine', () => {
  it('returns the transform unchanged when node has no text content', () => {
    const state = makeState();
    const tr = state.tr;
    const node = { descendants: () => undefined } as any;
