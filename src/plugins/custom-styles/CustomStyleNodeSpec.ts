...
function applyTextFormattingStyle(style: string, styles): string {
  ...
  if (styles.fontSize) {
    nextStyle += `font-size: ${styles.styles.fontSize}pt;`;
    if (styles.styles.fontSize !== EMPTY_STYLE_VALUE) {
      nextStyle += `--czi-counter-font-size: ${styles.styles.fontSize}pt;`;
    }
  }
  if (styles.fontName) {
    nextStyle += `font-family: ${styles.styles.fontName};`;
    if (styles.styles.fontName !== EMPTY_STYLE_VALUE) {
      nextStyle += `--czi-counter-font-family: ${styles.styles.fontName};`;
    }
  }
  return nextStyle;
}
