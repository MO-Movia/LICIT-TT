...
describe('Info Icon Command', () => {
  ...
  it('should compute parent positions', () => {
    ...
    expect(cmd.getParentStartPos(view.state.selection.$head)).toBeGreaterThanOrEqual(1);
  });
  ...
  it('InfoIconView basic behaviors', () => {
    ...
    const cView = new InfoIconView(view.state.doc.nodeAt(7), view, undefined as any);
  ...
  it('waitForUserInput returns when popup exists', async () => {
    ...
    const cmd = new InfoIconCommand();
    cmd._popUp = {close: jest.fn()} as any;
